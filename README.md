...
- сначала показывает 2 кнопки:
  ...
  - `Поделиться картой портфеля` — публичная share-карточка `1080x1350` (light glass, без абсолютных сумм);
- для share-карточки выводятся:
  - состав портфеля (доли + месячная динамика в процентах),
  - аллокация по категориям (акции/металлы/другое) в donut-диаграмме,
  - топ-3 лучших и топ-3 худших активов по процентам,
